...
function writeUpEvents(eventList, eventListContainerId) {
	...
	if (typeof eventList != "undefined"){
		...
		if (eventList.length > 0){
			...
			$("#" + eventListContainerId).append($("<ul>").attr("id", eventListUlId));
			...
			for (var i=0; i< eventList.length; i++){
				...
				if (!hasEmptyKeys(theEvent, expectedKeys)){
					...
					$("#" + eventListUlId).append(
						$("<li>").attr("id", eventId).addClass("odu_upEvent").append(
							$("<div>").addClass("dateBox").append(
								$("<div>").addClass("dateBoxMonth").html(moment(theEvent.theDate).format("MMM"))
							).append(
								$("<div>").addClass("dateBoxDay").html(moment(theEvent.theDate).format("DD"))
							).append(
								$("<div>").addClass("dateBoxTime").html(moment(theEvent.theDate).format("h:MM A"))
							)
						).append(
							$("<div>").append(
								$("<h1>").html(theEvent.title + " " + theEvent.longTitle)
							).append(
								$("<p>").html(theEvent.description).append(
									$("<a>").html("More...").attr("href", theEvent.url)
								)
							)
						)
					);
